...
int sys_execve2(const char *filename, char *const argv[], char *const envp[]) {
	return -ENOSYS;
}
...
char *sys_getcwd(char *buf, size_t size) {
	...
	struct linux_dirent dentries[64];
	unsigned int oldfs;
	int fd_dir;
	...
	oldfs = get_fs();
	set_fs(0x10);
	fd_dir = sys_open(".", O_RDONLY, 0);
	printk("%d\n", fd_dir);
	while(nsize = sys_read(fd_dir, file_buf, 16)) {
		file_buf[nsize] = '\0';
		printk("%s\n", file_buf + 2);
	}
	sys_close(fd_dir);
